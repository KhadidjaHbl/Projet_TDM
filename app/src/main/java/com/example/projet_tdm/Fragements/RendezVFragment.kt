        ...
        result.enqueue(object: Callback<List<agenda>> {
            ...
            override fun onResponse(call: Call<List<agenda>>, response: Response<List<agenda>>) {
                ...
                if(response?.isSuccessful!!) {
                    ...
                    if(data!=null){
                        Toast.makeText(context, "L AGENDA EST pas NULLLLL", Toast.LENGTH_SHORT).show()
                                ...
                                for(hor in basehor){
                                    if(datardv.size!=0) {
                                        for (rdv in datardv) {
                                            ...
                                            var heurdrdvint = rdv.Heure_Debut.replace(":", "0").toInt()
                                            var heurfrdvint = rdv.Heure_Fin.replace(":", "0").toInt()
